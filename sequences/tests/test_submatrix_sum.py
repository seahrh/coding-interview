...
class TestSubmatrixSum(unittest.TestCase):
    def test_given_empty_matrix_then_raise_error(self):
        self.assertRaises(ValueError, max_submatrix, [[]])

    def test_given_1x1_matrix(self):
        self.assertEqual(max_submatrix([[1]]),
                         Submatrix(
    ...
    def test_given_2x2_matrix(self):
        self.assertEqual(max_submatrix([[1, -2], [10, 2]]),
                         Submatrix(
    ...
    def test_given_2x3_matrix(self):
        self.assertEqual(max_submatrix([[1, -2, 10], [10, -2, 2]]),
                         Submatrix(
                             top_left=Cell(row=0, col=0),
                             bottom_right=Cell(row=1, col=2),
                             sum=19)
                         )
